...
public interface HistoryRepository extends JpaRepository<HistoryItemDTO, Integer> {
    ...
    @Query("SELECT c FROM HistoryItemDTO c JOIN AidDTO a ON c.aid_id = a.id WHERE (c.user_id=:id AND c.status=1) AND " +
    ...
    @Query("SELECT COUNT(c) FROM HistoryItemDTO c JOIN AidDTO a ON c.aid_id = a.id WHERE (c.user_id=:id AND c.status=1) AND " +
            "(LOWER(a.name) LIKE LOWER(CONCAT('%', :search, '%')) OR LOWER(a.description) LIKE LOWER(CONCAT('%', :search, '%')) " +
